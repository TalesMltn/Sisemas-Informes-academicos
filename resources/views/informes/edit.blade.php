...
<div class="min-h-screen bg-cover bg-center flex items-center justify-center p-6"
     style="background-image: url('{{ asset('image/ciiiinformes.png') }}');">
    ...
    <div class="w-full min-h-screen bg-black/50 flex items-center justify-center backdrop-blur-sm">
        ...
        <div class="bg-white/10 backdrop-blur-md p-10 rounded-3xl shadow-2xl w-full max-w-lg border border-white/30 text-white">
            ...
            {{-- Formulario --}}
            <form action="{{ route('informes.update', $informe->id) }}" method="POST" class="space-y-6 text-lg">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <select name="estudiante_id"
                            class="w-full bg-white/20 border border-white/30 p-3 rounded-md text-white focus:ring-2 focus:ring-yellow-300 focus:outline-none appearance-none"
                            style="color: #f9fafb;">
                        <option value="" class="text-gray-900 bg-white">-- Seleccione un estudiante --</option>
                        @foreach($estudiantes as $estudiante)
                            <option value="{{ $estudiante->id }}" 
                                    {{ old('estudiante_id', $informe->estudiante_id) == $estudiante->id ? 'selected' : '' }}
                ...
                <div>
                    ...
                    <input type="text" name="titulo"
                           ...
                           value="{{ old('titulo', $informe->titulo) }}" required>
                </div>
                ...
                <div>
                    ...
                    <textarea name="descripcion"
                              class="w-full bg-white/20 border border-white/30 p-3 rounded-md text-white placeholder-gray-200 focus:ring-2 focus:ring-yellow-300 focus:outline-none"
                              rows="4" required>{{ old('descripcion', $informe->descripcion) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="fecha"
                           class="w-full bg-white/20 border border-white/30 p-3 rounded-md text-white focus:ring-2 focus:ring-yellow-300 focus:outline-none"
